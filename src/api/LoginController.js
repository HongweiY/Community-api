...
class LoginController {
    ...
    async login(ctx) {
        ...
        if (codeCheck) {
            ...
            if (userCheck) {
                let token = jsonwebtoken.sign({_id: 'YHW'}, config.JWT_SECRET, {expiresIn: '24h'})
                ctx.body = {
                    code: 200,
                    token: token
                }
